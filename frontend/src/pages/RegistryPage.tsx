...
export default function RegistryPage({ drillDown, onDrillDownApplied }: RegistryPageProps = {}) {
  ...
  const params = useMemo(() => {
    ...
    return p.toString();
  }, [page, search, cycle, status, sphere, type]);
    ...
    <div className="registry-card">
      ...
      {isLoading ? (
              ...
              <tbody>
                {rows.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>{item.due_raw || "—"}</td>
                    <td>
                      <span className={statusClass(item.status_normalized)}>
                        {normalizeLabel(item.status_normalized) || "Нет статуса"}
                      </span>
                    </td>
                    <td>
                      <div className="action-row">
                        <button className="btn-link" onClick={() => setDetailItem(item)}>
                          Подробнее
                        </button>
        ...
        <div className="modal-backdrop" onClick={() => setDetailItem(null)}>
          <div className="modal-card modal-card-lg" onClick={(e) => e.stopPropagation()}>
            <div className="modal-title">Подробно по записи #{detailItem.seq_no || detailItem.id}</div>

            <div className="record-details">
              <div className="record-row">
                <div className="record-label">П/п</div>
                <div className="record-value">{detailItem.seq_no || detailItem.id || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Анализ / мониторинг</div>
                <div className="record-value">{detailItem.record_type_normalized || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">ЦИКЛ</div>
                <div className="record-value">{detailItem.cycle || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Сфера</div>
                <div className="record-value">{detailItem.sphere_normalized || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Предложения</div>
                <div className="record-value record-value-long">{detailItem.proposal_text || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Ответственный исполнитель</div>
                <div className="record-value">{detailItem.responsible_org || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Заинтересованные государственные органы</div>
                <div className="record-value record-value-long">{detailItem.interested_orgs || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Форма завершения</div>
                <div className="record-value record-value-long">{detailItem.completion_form || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Срок исполнения</div>
                <div className="record-value">{detailItem.due_raw || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Статус ГО</div>
                <div className="record-value">{normalizeLabel(detailItem.status_normalized) || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Позиция ГО на 2024-2025 гг.</div>
                <div className="record-value record-value-long">{detailItem.position_2024_2025 || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Позиция ГО на 27.03.2026</div>
                <div className="record-value record-value-long">{detailItem.position_2026 || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Позиция АДГС</div>
                <div className="record-value record-value-long">{detailItem.adgs_position || "—"}</div>
              </div>

              <div className="record-row">
                <div className="record-label">Кейс</div>
                <div className="record-value record-value-long">{detailItem.case_text || detailItem.case_note || "—"}</div>
              </div>
            </div>

            <div className="modal-actions">
              <button className="btn-primary" onClick={() => setDetailItem(null)}>
                Закрыть
              </button>
            </div>
          </div>
      ...
      {editItem && (
        <div className="modal-backdrop" onClick={() => setEditItem(null)}>
          <div className="modal-card modal-card-lg" onClick={(e) => e.stopPropagation()}>
            <div className="modal-title">Обновление статуса #{editItem.seq_no || editItem.id}</div>
            ...
            <div className="modal-actions">
              ...
              <button className="btn-primary" onClick={saveEdit} disabled={mutation.isPending}>
